...
class EntropyCalculatorApp(ctk.CTk):
    ...
    def create_inputs(self, size, channel_type):
        ...
        for i in range(size):
            row = []
            for j in range(size):
                row.append(random.uniform(0.1, 1.0))
            raw_matrix.append(row)
        if channel_type == "b|a":
            norm_matrix = self.normalize_matrix_rows(raw_matrix)
        elif channel_type == "a|b":
            norm_matrix = self.normalize_matrix_cols(raw_matrix)
        elif channel_type == "a,b":
            total_sum = sum(sum(row) for row in raw_matrix)
            norm_matrix = [[p / total_sum for p in row] for row in raw_matrix]
        for i in range(size):
            row_entries = []
            for j in range(size):
                entry = ctk.CTkEntry(frame_matrix, width=50, placeholder_text="0.00")
                entry.insert(0, f"{norm_matrix[i][j]:.3f}")
        ...
        if channel_type != "a,b":
            ...
            raw_vector = [random.uniform(0.1, 1.0) for _ in range(size)]
            norm_vector = self.normalize_vector(raw_vector)
            for i in range(size):
                entry = ctk.CTkEntry(frame_vector, width=50, placeholder_text="0.00")
                entry.insert(0, f"{norm_vector[i]:.3f}")
    ...
    def normalize_vector(self, vec):
        ...
        normalized = [v/s for v in vec]
        rounded = [round(x, 3) for x in normalized]
        total = sum(rounded)
    ...
    def normalize_matrix_cols(self, matrix):
        ...
        for j in range(m):
            ...
            if col_sum > 0:
                ...
                normalized[-1][j] = 1.0 - sum(normalized[i][j] for i in range(n-1))
            else:
                for i in range(n):
    ...
    def fill_random(self):
        ...
        for i in range(size):
            ...
            matrix.append(row)
        if channel_type == "b|a":
            matrix = self.normalize_matrix_rows(matrix)
        elif channel_type == "a|b":
            matrix = self.normalize_matrix_cols(matrix)
        elif channel_type == "a,b":
            total_sum = sum(sum(row) for row in matrix)
            matrix = [[p / total_sum for p in row] for row in matrix]
        for i in range(size):
            for j in range(size):
                self.matrix_entries[i][j].delete(0, "end")
                self.matrix_entries[i][j].insert(0, f"{matrix[i][j]:.3f}")
        if channel_type != "a,b":
            vector = [random.uniform(0.1, 1.0) for _ in range(size)]
            vector = self.normalize_vector(vector)
            for i in range(size):
                self.vector_entries[i].delete(0, "end")
                self.vector_entries[i].insert(0, f"{vector[i]:.3f}")
    def clear_all(self):
    ...
    def calculate_redundancy(self, H, H_max):
        if H_max <= 0:
            return 0.0
        return 1.0 - (H / H_max)
    def calculate_capacity(self, H_max, H_cond):
        if H_max <= H_cond:
            return 0.0
        return H_max - H_cond
    def calculate(self):
        ...
        result_lines.append(f"Расчет пропускной способности")
        result_lines.append(f"и избыточности:")
        if channel_type == "b|a":
            c = self.calculate_capacity(results['h_b'], results['h_b_given_a'])
            d = self.calculate_redundancy(results['h_a'], h_max)
            result_lines.append(f"C: {c:.3f}")
            result_lines.append(f"D: {d:.3f}")
        elif channel_type == "a|b":
            c = self.calculate_capacity(results['h_a'], results['h_a_given_b'])
            d = self.calculate_redundancy(results['h_b'], h_max)
            result_lines.append(f"C: {c:.3f}")
            result_lines.append(f"D: {d:.3f}")
        elif channel_type == "a,b":
            c = self.calculate_capacity(results['h_a'] + results['h_b'], results['h_ab'])
            d_a = self.calculate_redundancy(results['h_a'], h_max)
            d_b = self.calculate_redundancy(results['h_b'], h_max)
            result_lines.append(f"C: {c:.3f}")
            result_lines.append(f"D(A): {d_a:.3f}")
            result_lines.append(f"D(B): {d_b:.3f}")
        self.update_result_text("\n".join(result_lines))
